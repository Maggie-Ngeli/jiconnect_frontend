...
function Busdev() {
    ...
    const [waitingforqoute, setWaitingforqoute] = useState('0');
    const [waitingforsend, setWaitingforsend] = useState('0');
    const [ondesign, setOndesign] = useState('0');
    ...
    useEffect(() => {
        fetch('http://127.0.0.1:8004/api/applications/applications/all/', {
            'method':'GET',
            headers: {
    ...
    useEffect(() => {
        if(data==null) {
            ...
            setWaitingforqoute(data.waiting_for_qoute)
            setWaitingforsend(data.waiting_for_send)
            setOndesign(data.on_design)
            setSitesvisited(data.total_applications)
            setSitesvisited(data.sites_visited)
    ...
    function getBase64(file, applicationid) {
        ...
        reader.onload = function () {
          APIService.UpdateApplication(applicationid, {
            "id": applicationid,
            "qoutation": reader.result,
            "app_status": "Waiting for Payment",
          })
          .then(() => setOriApplications(oriapplications))
          .catch(error => console.log(error))
     ...
     function changeStatus(applicationid) {
        APIService.UpdateApplication(applicationid, {
            "id": applicationid,
            "app_status": "On Design",
        });
        setAppStatus("On Design");
     }
    ...
    return (
        ...
        <MainStats>
            ...
            <StatsContainer>
                <h5>Applications Waiting for Qoute</h5>
                <hr />
                <h4>{waitingforqoute}</h4>
            </StatsContainer>
            <StatsContainer>
                <h5>Applications Under Review</h5>
                <hr />
                <h4>{waitingforsend}</h4>
            </StatsContainer>
            <StatsContainer>
                <h5>Applications On Design</h5>
                <hr />
                <h4>{ondesign}</h4>
            </StatsContainer>
            <StatsContainer>
                <h5>On Sites Visited</h5>
                <hr />
                <h4>{sitesvisited}</h4>
            </StatsContainer>
            <StatsContainer>
        ...
        <FilterContainers className="d-block">
                <FilterNav className="d-flex"> 
                    ...
                    <div className="d-flex">
                            <Link to="/applications/all" onClick={() => setCounties('all')}><a className="browse_category active_category cat_all">All</a></Link>
                            <Link to="/applications/all/mombasa" onClick={() => setCounties('mombasa')}><a className="browse_category cat_mombasa">Mombasa</a></Link>
                            <Link to="/applications/all/kwale" onClick={() => setCounties('kwale')}><a className="browse_category cat_kwale">Kwale</a></Link>
                            <Link to="/applications/all/kalifi" onClick={() => setCounties('kalifi')}> <a className="browse_category cat_kalifi">Kalifi</a></Link>
                            <Link to="/applications/all/tanariver" onClick={() => setCounties('tanariver')}><a className="browse_category cat_tanariver">Tana River</a></Link>
                            <Link to="/applications/all/taitataveta" onClick={() => setCounties('taitataveta')}><a className="browse_category cat_taitataveta">Taita Taveta</a></Link>
                        </div>
                    </FilterCategories>
                            ...
                            <FilterContainer>
                                <div className="downloads_filter">
                                    ...
                                    <Dropdown>
                                        ...
                                        <Dropdown.Menu>
                                            <TypeItem onClick={() => setAppStatus("Under Review")}>
                                                <Dropdown.Item>Under Review</Dropdown.Item>
                                            </TypeItem>
                                            <TypeItem onClick={() => setAppStatus("Waiting for Qoute")}>
                                                <Dropdown.Item>Waiting for Qoute</Dropdown.Item>
                                            </TypeItem>
                                            <TypeItem onClick={() => setAppStatus("Waiting for Payment")}>
                                                <Dropdown.Item>Waiting for Payment</Dropdown.Item>
                                            </TypeItem>
                                            <TypeItem onClick={() => setAppStatus("On Design")}>
                                                <Dropdown.Item>On Design</Dropdown.Item>
                                            </TypeItem>
                                            <TypeItem onClick={() => setAppStatus("Payment Settled")}>
                                                <Dropdown.Item>Payment Settled</Dropdown.Item>
                                            </TypeItem>
                                            <TypeItem onClick={() => setAppStatus("Site Visited")}>
                                                <Dropdown.Item>Site Visited</Dropdown.Item>
                                            </TypeItem>
            ...
            <table class="table">
                ...
                { applications.map(application => {
                    ...
                    <tr key = {application.id}>
                        ...
                        <td>{(String(application.proposal).includes("null")) ? <img src="/images/waiticon.svg"/> : <img src="/images/agreedicon.svg"/>}</td>
                        <td>
                            <Link to={`/documents/${application.customer}`}>
                              <a className="docs_btn">Open</a>
                            </Link>
                        </td>
                        <td>
                           
                            {(application.qoutation == 'null') ? 
                                <a className="upload_btn" >
                                    <img src="/images/attachmenticon.svg"/>
                                    Upload Qoute
